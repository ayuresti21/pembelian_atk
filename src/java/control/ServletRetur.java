...
public class ServletRetur extends pesan {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            retur var = new retur(noretur,tglretur,kdsupplier,kdbarang,qty,subtotal);
    ...
    result2=pstmt2.executeUpdate();
        if (result +result1 + result2 > 0) {
            out.println("<script> "
                    ...
                    + "document.location='beranda.jsp?halaman=retur';"
                    + " </script>");
        }
            }
        
            } catch (ClassNotFoundException ex) {
                Logger.getLogger(ServletRetur.class.getName()).log(Level.SEVERE, null, ex);
